...
class RandomWalk(Node):
    ...
    def timer_callback(self):
        ...
        back_right_lidar_min = min(self.scan_cleaned[RIGHT_SIDE_INDEX:BACK_RIGHT_INDEX])
        ...
        elif (back_right_lidar_min < 1 and right_lidar_min > 1.2):
            self.cmd.linear.x = 0.0
            self.cmd.angular.z = -0.89
            self.publisher_.publish(self.cmd)
            self.get_logger().info('Room Enter???')
